...
import { ArrowUpDown, ExternalLink, Search, Settings, Trash2 } from "lucide-react";
...
const MOCK_USD: Record<string, number> = { ETH: 2700, USDC: 1, USDT: 1, ULTRA: 2.1 };
...
export default function SwapPage() {
  ...
  const [tempSlip, setTempSlip] = useState<number>(((slippageTolerance ?? 50) as number) / 100);
  const [slippagePct, setSlippagePct] = useState<number>(((slippageTolerance ?? 50) as number) / 100);
  ...
  const canSwap = useMemo(
    () => Boolean(tokenIn && tokenOut && (amountIn && Number(amountIn) > 0) && !isSwapping),
  ...
  const handleSwap = async () => {
    ...
    setTimeout(() => {
      setIsSwapping(false);
      setHistory((h) =>
        h.map((row) => (row.id === id ? { ...row, status: "success", txHash: "0x" + id.slice(0, 16) } : row))
      );
    }, 1100);
  };
  ...
  const rateText = useMemo(() => {
    if (!tokenIn || !tokenOut) return "--";
    const r = (MOCK_USD[tokenIn] ?? 0) / Math.max(MOCK_USD[tokenOut] ?? 0, 1e-9);
      ...
      <div className="mx-auto max-w-md space-y-6">
        ...
        <Card className="rounded-3xl border border-neutral-800/60 bg-neutral-950/60 shadow-2xl">
          <CardHeader className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="icon" title="Settings" onClick={() => setOpenSettings(true)} className="cursor-pointer">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 rounded-2xl border border-neutral-800 bg-neutral-900 px-3 py-2">
                ...
                />
                <Button variant="outline" className="min-w-24 cursor-pointer" onClick={() => setOpenIn(true)}>
                  {tokenIn || "Select"}
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 rounded-2xl border border-neutral-800 bg-neutral-900 px-3 py-2">
                <Input
                  ...
                  className="flex-1 bg-transparent text-right text-2xl font-semibold"
                />
                <Button variant="outline" className="min-w-24 cursor-pointer" onClick={() => setOpenOut(true)}>
                  {tokenOut || "Select"}
                </Button>
              </div>
            </div>
        ...
        <Card className="rounded-3xl border border-neutral-800/60 bg-neutral-950/60">
          <CardHeader className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onClick={() => setHistory([])} disabled={!history.length} className="cursor-pointer">
                  ...
                  {history.length === 0 ? (
                    ...
                    history.map((h) => (
                      <TableRow key={h.id}>
                        ...
                        <TableCell className="text-right font-mono tabular-nums">{fmt(h.amountIn)}</TableCell>
                        <TableCell className="text-right font-mono tabular-nums">{fmt(h.amountOut)}</TableCell>
                        <TableCell className="text-right">
                          {h.status === "success" ? (
                            <a
                              className="inline-flex items-center gap-1 text-emerald-400 hover:underline"
                              href={h.txHash ? `https://etherscan.io/tx/${h.txHash}` : undefined}
                              target="_blank"
                              rel="noreferrer"
                            >
        ...
        <Dialog open={openSettings} onOpenChange={setOpenSettings}>
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-3">
              <div className="text-sm text-neutral-400">Slippage tolerance(%)</div>
              <div className="flex items-center gap-2">
                <Input
                  ...
                  onChange={(e) => setTempSlip(Math.max(0, Number(e.target.value)))}
                  className="flex-1"
                />
                <div className="grid grid-cols-3 gap-2">
                  {[0.1, 0.5, 1].map((v) => (
                    <Button key={v} variant="outline" size="sm" onClick={() => setTempSlip(v)} className="cursor-pointer">
                      {v}%
                    </Button>
                  ))}
                </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button variant="ghost" onClick={() => setOpenSettings(false)} className="cursor-pointer">Cancle</Button>
                <Button
                  onClick={() => {
                    ...
                    if ((useSwapStore as any)().setSlippageTolerance) {
                      // @ts-ignore
                      (useSwapStore as any)().setSlippageTolerance(Math.round(tempSlip * 100));
                    }
      ...
      <Dialog open={openIn} onOpenChange={setOpenIn}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-neutral-800 bg-neutral-900 px-3 py-2">
            ...
            <Input placeholder="심볼/주소 검색" value={search} onChange={(e) => setSearch(e.target.value)} className="border-0 bg-transparent" />
          </div>
          <div className="max-h-80 overflow-auto rounded-xl border border-neutral-800">
            {filteredTokens.map((t) => (
              ...
              >
                ...
                <div className="text-[11px] text-neutral-500">{t.address.slice(0, 6)}…{t.address.slice(-4)}</div>
              </button>
            ))}
